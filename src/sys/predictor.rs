...
use crate::bindings::{
    ...
    TreelitePredictorPredictBatch, TreelitePredictorQueryLeafOutputType, TreelitePredictorQueryNumClass, TreelitePredictorQueryResultSize,
};
...
pub fn predictor_load(library_path: &Path, num_worker_thread: usize) -> PredictorHandle {
...
#[throws(TreeRiteError)]
pub fn predictor_free(handle: PredictorHandle) {
...
#[throws(TreeRiteError)]
pub fn predictor_predict_batch(handle: PredictorHandle, batch: DMatrixHandle, verbose: bool, pred_margin: bool) -> (PredictorOutputHandle, u64) {
    let verbose = if verbose { 1 } else { 0 };
    let pred_margin = if pred_margin { 1 } else { 0 };
    let output_result = create_predictor_output_vector(handle, batch)?;
...
#[throws(TreeRiteError)]
pub fn predictor_query_leaf_output_type(handle: PredictorHandle) -> DataType {
    let mut out = null();
...
#[throws(TreeRiteError)]
pub fn predictor_query_num_class(handle: PredictorHandle) -> u64 {
    let mut out = 0;
...
#[throws(TreeRiteError)]
pub fn predictor_query_result_size(handle: PredictorHandle, batch: DMatrixHandle) -> u64 {
    let mut out = 0;
...
#[throws(TreeRiteError)]
pub fn create_predictor_output_vector(handle: PredictorHandle, batch: DMatrixHandle) -> PredictorOutputHandle {
    let mut out = null_mut();
...
#[throws(TreeRiteError)]
pub fn delete_predictor_output_vector(handle: PredictorHandle, output_vector: PredictorOutputHandle) {
